...
export const FilterMovieList: FC<FilterMovieListProp> = ({ movies }) => {

  const navigate = useNavigate()
    const {setFilterMovie, setIsFilterModal} = useFilterMovie()

  if (!movies || movies.length === 0) {
    return <p className="filter-movie__error">По Вашему запросу фильмы не найдены</p>;
  }
  ...
  return (
    <ul className="list-reset filter-movie__list">
      {movies.map((item) => {
        const backgroundColor = getBackgroundColorByRating(
          item.tmdbRating || 0
        );
        return (
          <li onClick={() => handleClick(item)} key={item.id} className="filter-movie__item">
